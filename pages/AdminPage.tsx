...
const AdminPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'about' | 'team' | 'pricing' | 'blog' | 'leads' | 'footer'>('blog');
  ...
  const InputPair = ({ label, path, isTextArea = false }: { label: string, path: string, isTextArea?: boolean }) => {
    const isImage = label.toLowerCase().includes('image') || label.toLowerCase().includes('icon') || label.toLowerCase().includes('slide') || label.toLowerCase().includes('logo');
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm space-y-4">
          ...
          <div className="flex gap-4 items-center">
            ...
            <input 
              ...
              className="flex-1 p-4 bg-gray-50 rounded-xl font-bold outline-none border border-transparent focus:border-teal-100" 
    ...
    <div className="min-h-screen bg-[#f8fafc] flex">
      <div className="w-80 bg-[#0c0c0c] flex flex-col fixed h-full z-[100] shadow-2xl">
        ...
        <nav className="flex-1 p-6 space-y-1 overflow-y-auto hide-scrollbar">
          {['about', 'team', 'pricing', 'blog', 'leads', 'footer'].map(id => (
                ...
                <div key={m.id} className={`bg-white p-10 rounded-[4rem] border transition-all ${m.isActive === false ? 'opacity-60 grayscale border-red-100' : 'border-gray-100 shadow-sm'} space-y-6`}>
                  <div className="flex justify-between items-center border-b border-gray-50 pb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">
                        {m.isActive !== false ? 'აქტიური (ჯავშანში ჩანს)' : 'გათიშული (ჯავშანში არ ჩანს)'}
                      </span>
                    </div>
                    <button 
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-4 items-center">
                      {m.image && (
                        <div className="w-20 h-20 rounded-2xl overflow-hidden border border-gray-100 shrink-0">
                          <img src={m.image} className="w-full h-full object-cover" alt="Preview" />
                        </div>
                      )}
                      <input 
                        value={m.image} 
                        onChange={e => { const nt = [...team]; nt[idx].image = e.target.value; setTeam(nt); }} 
                        className="flex-1 p-4 bg-gray-50 rounded-xl font-bold outline-none" 
                        placeholder="https://..."
                      />
                    </div>
                  </div>
